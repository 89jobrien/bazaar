        ...
        map.entry(p.name.clone())
            .and_modify(|existing| {
                ...
                if existing.pushed_at.is_none() {
                    existing.pushed_at = p.pushed_at;
                }
                if existing.version.is_none() {
                    existing.version = p.version.clone();
                }
                ...
                if existing.recent_commits.is_empty() {
                    existing.recent_commits = p.recent_commits.clone();
                }
                if existing.topics.is_empty() {
                    existing.topics = p.topics.clone();
                }
                if existing.readme.is_none() {
                    existing.readme = p.readme.clone();
                }
                if existing.category.is_none() {
                    existing.category = p.category.clone();
                }
                if existing.changelog.is_none() {
                    existing.changelog = p.changelog.clone();
                }
                if existing.health.is_none() {
                    existing.health = p.health.clone();
                }
                if existing.related.is_empty() {
                    existing.related = p.related.clone();
                }
            })
            .or_insert(p);
    }
